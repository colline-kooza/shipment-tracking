...
export async function checkAndNotifyDelayedShipments(): Promise<NotificationResult> {
  try {
    ...
    for (const shipment of delayedShipments) {
      ...
      if (!shipment.client && !shipment.Customer?.email) {
        // Handle the case (e.g., skip or throw error)
        continue;
      }
      ...
      const notificationData = {
        shipment: {
          ...
          client: shipment.client ?? "Unknown Client", // 👈 default fallback
        ...
        customerName: shipment.Customer?.name || shipment.client || "Customer",
      ...
      await db.notification.create({
        data: {
          ...
          message: `Your shipment is delayed by ${daysDelayed} day${
            daysDelayed > 1 ? "s" : ""
          }`,
      ...
      staffMembers.map((staff) =>
        db.notification.create({
          data: {
            ...
            message: `Status changed from ${previousStatus.replace(
              /_/g,
              " "
